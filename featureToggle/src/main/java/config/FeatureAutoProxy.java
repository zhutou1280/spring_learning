...
public class FeatureAutoProxy extends AbstractAutoProxyCreator {
     ...
     *
     * @return id of {@link FeatureToggleAspect} bean
     */
    private String getBeanNameOfFeatureAdvisor() {
        return FeatureToggleAspect.class.getAnnotation(Component.class).value();
    }
     ...
     *
     * @param currentInterface class to be scanned
     * @return
     */
    ...
    private Object[] scanInterfaceForAnnotation(Class<?> currentInterface, String currentInterfaceName) {
        ...
        if (AnnotatedElementUtils.hasAnnotation(currentInterface, FeatureToggle.class)) {
            ...
            for (Method method : currentInterface.getDeclaredMethods()) {
                if (AnnotatedElementUtils.hasAnnotation(method, FeatureToggle.class)) {
                    processedInterface.put(currentInterfaceName, true);
                    return PROXY_WITHOUT_ADDITIONAL_INTERCEPTORS;
                }
            }
